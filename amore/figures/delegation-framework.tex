...
\begin{tikzpicture}[
    ...
    \draw[arrow] (server.south) -- ++(0,-1.5) -| node[below, pos=0.25, align=center] {$\rho = e(A,B) \in \Group_T$\\$\gamma \subset \Group_T$ (proof)} (client.south);
    \node[stepnum] at (4,-1.6) {2};
    
    % Verification box with step number
    \node[draw, dashed, rounded corners, minimum width=2.5cm, minimum height=0.8cm, right=1.5cm of client, align=center, font=\small] (verify) {Verify $(\rho,\gamma)$};
