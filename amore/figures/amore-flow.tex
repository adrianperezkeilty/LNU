...
\begin{tikzpicture}[scale=0.8, every node/.style={transform shape},
    ...
    % Setup phase
    \node[clientbox, below=0.5cm of loopstart] (setup) {\textbf{Setup}\\$\state, \vec{A}, \vec{B} \to \pubvalue, \secvalue$};
    ...
    % Success output
    \node[left=1cm of result, label, text=green!60!black] (success) {Output $\vec{\rho}$\\Continue loop};
    
    % Failure output
    \node[right=5cm of result, label, text=red!70!black] (fail) {Abort\\Restart from\\OneTimeSetup};
    
    % Arrows
    \draw[arrow] (onetime) -- (loopstart);
    \draw[arrow] (loopstart) -- (setup);
